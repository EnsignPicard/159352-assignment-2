/*
flight search page
searches flights and handles bookings
*/
...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/routes")
        ...
        .then((data: RouteInfo[]) => {
          ...
          let firstRoute = data.find((route) => {
            return route.orig === "NZNE";
          });

          if (firstRoute) {
            setDest(firstRoute.dest);
          }
  ...
  let validDestCodes = routes
      .filter((route) => {
        return route.orig === orig;
      })
      .map((route) => {
        return route.dest;
      });

  let destOptions = airports.filter((airport) => {
    return validDestCodes.includes(airport.code);
  });
  ...
  const handleOrigChange = (value: string) => {
    ...
    let newValidDests = routes
        .filter((route) => {
          return route.orig === value;
        })
        .map((route) => {
          return route.dest;
        });
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              ...
              <div className="mb-4">
                ...
                <p className="text-sm text-gray-500">
                  {results.aircraft} · ${results.price} NZD per seat
                </p>
